...
import { SubscriptionPlan, UpdatePlanDto } from '../types';
...
export function EditPlanModal({ open, onOpenChange, plan, onSuccess }: EditPlanModalProps) {
  ...
  const [pricingOptions, setPricingOptions] = React.useState(plan.pricing);
  ...
  React.useEffect(() => {
    if (plan) {
      ...
      setPricingOptions(plan.pricing);
    }
  }, [plan, form]);

  const handleSubmit = async (values: PlanFormValues) => {
    try {
      await updatePlan.mutateAsync({ id: plan.id, data: values as UpdatePlanDto });
      form.reset();
      onSuccess?.();
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the subscription plan details, pricing and limits
          </DialogDescription>
        </DialogHeader>
